...
int dfs(TreeNode* root) {
	...
	if (root == NULL) {
		return -1; // hgt of empty tree
	}

	// recursive case

	// f(root) = find the hgt of the given tree

	// 1. ask your friend to find the hgt of LST

	int hLST = dfs(root->left);

	// 2. ask your friend to find the hgt of RST

	int hRST = dfs(root->right);
